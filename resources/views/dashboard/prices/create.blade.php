<x-layouts::dashboard>
    <x-components::status />
    <div class="header fs-1 fw-bold w-100 mx-auto text-center"> {{__("Add Price")}}</div>
    <form action="{{ route('dashboard.prices.store') }}" method="POST">
        @csrf
        <div class="row m-3">
            <div class="mb-3 col">
                <label class="form-label">{{__('Name')}}</label>
                <input type="text" class="form-control" name="name" value="{{$template->name}}" placeholder="{{__('Enter'). " " .__('Name')}}" />
            </div>
        </div>
        <div class="row m-3">    
            <div class="mb-3 col">
                <label class="form-label">{{__('Arabic App Description')}}</label>
                <input type="text" class="form-control" name="app_description_ar" value="{{$template->app_description_ar}}" placeholder="{{__('Enter')}} {{__('Arabic App Description')}}" />
            </div>
            <div class="mb-3 col">
                <label class="form-label">{{__('English App Description')}}</label>
                <input type="text" class="form-control" name="app_description_en" value="{{$template->app_description_en}}" placeholder="{{__('Enter')}} {{__('English App Description')}}" />
            </div>
        </div>
        </div>
        <div class="row m-3">
            <div class="mb-3 col">
                ...
                <textarea type="text" rows="10" class="form-control" name="app_description_detailed_ar" value="{{$template->app_description_detailed_ar}}" placeholder="{{__('Enter')}} {{__('Arabic App Description Detailed')}}">{{$template->app_description_detailed_ar}}</textarea>
            </div>
            <div class="mb-3 col">
                <label class="form-label">{{__('English App Description Detailed')}}</label>
                <textarea type="text" rows="10" class="form-control" name="app_description_detailed_en" placeholder="{{__('Enter')}} {{__('English App Description Detailed')}}">{{$template->app_description_detailed_en}}</textarea>
            </div>
        </div>
        <div class="mb-3 mx-2">
            <label class="form-label">{{__('Free Time')}}</label>
            <input type="number" class="form-control" name="free_time" required value="{{$template->free_time}}" placeholder="{{__('Enter')}} {{__('Free Time')}}" />
        </div>
        <div class="mb-3 mx-2">
            <label class="form-label">{{__('Max Hours')}}</label>
            <input type="number" class="form-control" name="max_hours" required value="{{$template->max_hours}}" placeholder="{{__('Enter')}} {{__('Max Hours')}}" />
        </div>
        <div class="mb-3 mx-2">
            <label class="form-label">{{__('Insurance Amount')}}</label>
            <input type="number" class="form-control" name="insurance" required value="{{$template->insurance}}" placeholder="{{__('Enter')}} {{__('Insurance Amount')}}" />
        </div>
        {{-- Price details --}}
        <div class="mb-3 mx-2">
            <label class="form-label fs-2 w-100 d-block text-center">{{__('Price Details')}}</label>
            {{-- <input type="hidden" name="prices" required value="{{$template->prices}}"/> --}}
            @foreach (json_decode($template->prices,true) as $type => $prices)
            <div id="{{__($type)}}">      
