<x-layouts::dashboard>
    <x-components::status />
    <h2 class="text-center fw-bold mx-auto">{{__('Edit Voucher')}} #{{$voucher->id}}</h2>
    <form action="{{route('dashboard.vouchers.update', $voucher->id)}}" method="POST" enctype="multipart/form-data">
    @csrf
    @method("PUT")
    <div class="mb-3">
        <label class="form-label">{{__('Code')}}</label>
        <input type="text" class="form-control" name="code" required value="{{$voucher->code}}" placeholder="Enter {{__('Code')}}" />
    ...
    <div class="mb-3">
        ...
        <div class="form-selectgroup">
            <label class="form-selectgroup-item flex-fill">
                <input type="radio" name="type" value="0" class="form-selectgroup-input" {{$voucher->type == 0 ? 'checked' : ''}} />
                <span class="form-selectgroup-label">{{ __('Percentage') }}</span>
            </label>
            <label class="form-selectgroup-item flex-fill">
                <input type="radio" name="type" value="1" class="form-selectgroup-input" {{$voucher->type == 1 ? 'checked' : ''}} />
    ...
    <div class="mb-3">
        ...
        <input type="number" class="form-control" name="value" required value="{{$voucher->value}}" placeholder="Enter {{__('Value')}}" />
    </div>
    {{-- Users --}}
    <div class="mb-3">
